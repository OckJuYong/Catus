# 🎯 사용자 플로우 명세서

> Catus - 감정 일기 챗봇 서비스

---

## 1. 신규 사용자 플로우

### 최초 로그인
```
LoginPage (/)
    ↓
[카카오 로그인]
    ↓
카카오 인증 (외부)
    ↓
KakaoCallbackPage
    ↓
POST /api/auth/kakao
    ↓
├─ isNewUser: true → 온보딩
└─ isNewUser: false → HomePage
```

### 온보딩
```
OnboardingPage
    ↓
[닉네임, 비밀번호(선택), 일기시간 입력]
    ↓
POST /api/auth/signup
    ↓
Onboarding
    ↓
[성별, 연령, 직업, 목적 입력]
    ↓
HomePage → Tutorial 자동 시작
```

---

## 2. 일상 사용 플로우

### 채팅 및 일기 생성
```
HomePage
    ↓
"오늘 하루는 어땠어?" 클릭
    ↓
ChatPage
    ↓
[AI와 대화] → IndexedDB 저장
    ↓
[저녁 - 설정 시간]
    ↓
"일기 쓰기 완료" 버튼
    ↓
DiaryRevealPage
    ↓
POST /api/diary/generate { date, chatLogs }
    ↓
일기 생성 완료 (애니메이션)
    ↓
DiaryDetailPage
```

### IndexedDB 저장 흐름
```
사용자 메시지 입력
    ↓
saveChatMessage(todayKey, userMessage, synced: false)
    ↓
POST /api/chat/message
    ↓
AI 응답 수신
    ↓
saveChatMessage(todayKey, aiMessage, synced: false)
    ↓
[일기 생성 후]
    ↓
markMessagesAsSynced(todayKey)
```

---

## 3. 일기 관리 플로우

### 캘린더 조회
```
HomePage → [일기장 아이콘]
    ↓
CalendarPage
    ↓
GET /api/diary/list?year&month
    ↓
감정 이모지로 날짜 표시
    ├─ 😊 행복
    ├─ 😢 슬픔
    ├─ 😐 보통
    ├─ 😠 화남
    └─ 😰 불안
    ↓
[날짜 클릭]
    ↓
DiaryDetailPage
```

### 일기 수정/삭제
```
DiaryDetailPage
    ↓
├─ [수정] → PUT /api/diary/{id}
└─ [삭제] → DELETE /api/diary/{id} → CalendarPage
```

---

## 4. 익명 메시지 플로우

### 랜덤 일기 및 메시지 전송
```
HomePage → [고양이 "달이" 클릭]
    ↓
RandomDiaryPage
    ↓
GET /api/diary/random
    ↓
├─ 일기 없음 → "아직 공유된 일기가 없어요 😢"
└─ 일기 있음 → 일기 표시
        ↓
    [응원 메시지 보내기]
        ↓
    MessageModal (최대 100자)
        ↓
    POST /api/message/send
        ↓
    "응원 메시지를 전달했어요! 😊"
```

### 받은 메시지 확인
```
HomePage
    ↓
[새 메시지 도착]
    ↓
종이비행기 배지 표시 (unreadCount)
    ↓
종이비행기 애니메이션 (위아래)
    ↓
[종이비행기 클릭]
    ↓
LetterPage
    ↓
GET /api/message/received
    ↓
메시지 목록 표시
    ↓
setLastCheckedCount() → 배지 숨김
```

---

## 5. 성격 분석 플로우

### BIG5 초기 테스트
```
HomePage → [선인장 클릭]
    ↓
Big5StatsPage
    ↓
GET /api/big5/current
    ↓
├─ 점수 없음 → "검사 시작하기"
└─ 점수 있음 → 통계 화면
        ↓
    Big5TestPage
        ↓
    [10개 질문 5점 척도]
        ├─ 개방성 2문항
        ├─ 성실성 2문항
        ├─ 외향성 2문항
        ├─ 친화성 2문항
        └─ 신경성 2문항
        ↓
    진행률 바 (10% → 100%)
        ↓
    POST /api/big5/initial { answers[] }
        ↓
    Big5StatsPage (자동 이동)
```

### BIG5 통계 확인
```
Big5StatsPage
    ↓
GET /api/big5/current
    ↓
5가지 특성 프로그레스 바
    ├─ 개방성 (빨강)
    ├─ 성실성 (청록)
    ├─ 외향성 (노랑)
    ├─ 친화성 (민트)
    └─ 신경성 (보라)
    ↓
자동 업데이트 안내
"매주 자동 업데이트 (최대 0.15/week)"
```

---

## 6. 설정 관리 플로우

### 설정 변경
```
HomePage → [설정 아이콘]
    ↓
SettingsPage
    ↓
GET /api/settings
    ↓
├─ 일기 시간 → PUT /api/settings/diary-time
├─ 알림 설정 → PUT /api/settings/notifications
├─ 다크모드 → PUT /api/settings/theme
└─ 프로필 → PUT /api/settings/profile
```

### 회원 탈퇴
```
SettingsPage
    ↓
[회원 탈퇴 버튼]
    ↓
WithdrawModal
    ↓
"정말 탈퇴하시겠습니까?"
    ↓
[비밀번호 입력] (설정한 경우)
    ↓
DELETE /api/auth/withdraw
    ↓
localStorage.clear()
indexedDB.deleteDatabase('CatusDB')
    ↓
LoginPage
```

---

## 7. 에러 처리 플로우

### 네트워크 오류
```
API 요청 실패
    ↓
"네트워크 연결을 확인해주세요"
    ↓
[재시도 버튼]
```

### 인증 만료
```
API 401 응답
    ↓
Axios 인터셉터
    ↓
localStorage.removeItem('catus_access_token')
    ↓
window.location.href = '/'
    ↓
"인증이 만료되었습니다"
```

### 서버 오류
```
API 500 응답
    ↓
"서버 오류가 발생했습니다"
    ↓
React Query 자동 재시도 (1회)
```

---

## 8. 특수 플로우

### 자동 로그인
```
앱 접속 (/)
    ↓
AuthContext 초기화
    ↓
localStorage.getItem('catus_access_token')
    ↓
├─ 토큰 있음 → HomePage 자동 이동
└─ 토큰 없음 → LoginPage 유지
```

### 페이지 새로고침
```
브라우저 새로고침
    ↓
React 앱 재초기화
    ↓
├─ AuthContext → 토큰 복원
├─ DarkModeContext → 다크모드 복원
├─ TutorialContext → 튜토리얼 상태 복원
└─ ChatPage → IndexedDB 메시지 복원
```

---

## 9. 주요 시나리오

### Day 1 (신규 사용자)
```
1. 카카오 로그인
2. 온보딩 (닉네임, 일기 시간)
3. 홈 튜토리얼
4. 채팅 시작
5. 저녁 일기 생성
6. BIG5 테스트
```

### Day 7 (일상 사용)
```
1. 자동 로그인
2. 채팅
3. 일기 확인
4. 응원 메시지 확인
5. 지난 일기 조회
```

### 익명 교류
```
1. 고양이 클릭
2. 랜덤 일기 조회
3. 응원 메시지 전송
4. 내 일기에 메시지 도착
5. 종이비행기 배지 확인
6. 메시지 읽기
```

---

## 10. 플로우 다이어그램

```
┌──────────────┐
│  LoginPage   │
└──────┬───────┘
       │
       ├─ 신규 → OnboardingPage → HomePage (Tutorial)
       └─ 기존 → HomePage
                  │
      ┌───────────┼───────────┐
      │           │           │
  ChatPage   CalendarPage  Big5TestPage
      │           │           │
 IndexedDB   DiaryDetail  Big5Stats
      │           │
DiaryReveal  RandomDiary
      │           │
DiaryDetail  LetterPage
```

---

**문서 버전**: 1.0
**최종 수정**: 2025-01-21
